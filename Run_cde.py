# huberloss，又称平滑L1损失，是L1损失和L2损失的折中，当误差较小时，使用L2损失，当误差较大时，使用L1损失，这样可以减小异常值对损失函数的影响。
#loss = 0.5 * (y_true - y_pred)^2                  if |y_true - y_pred| <= delta
#loss = delta * |y_true - y_pred| - 0.5 * delta^2   otherwise
...
if args.loss_func == 'mask_mae':
    ...
    loss = torch.nn.HuberLoss(delta=1.0).to(args.device)     #Huber loss参数解释：delta指定阈值，大于阈值的误差用L1损失计算，小于阈值的误差用L2损失计算，这样可以减小异常值对损失函数的影响。reduction指定损失值的计算方式，可选值有none、mean、sum
else:
    raise ValueError
